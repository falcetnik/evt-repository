...
export function validateEnv(config: Record<string, unknown>): AppEnv {
  ...
  const publicInviteBaseUrl = String(config.PUBLIC_INVITE_BASE_URL);
  let parsedPublicInviteUrl: URL;
  try {
    parsedPublicInviteUrl = new URL(publicInviteBaseUrl);
  } catch {
    throw new Error('PUBLIC_INVITE_BASE_URL must be a valid absolute URL');
  }

  if (!['http:', 'https:'].includes(parsedPublicInviteUrl.protocol)) {
    throw new Error('PUBLIC_INVITE_BASE_URL must use http or https protocol');
  }
  ...
  return {
    ...
    PUBLIC_INVITE_BASE_URL: parsedPublicInviteUrl.toString().replace(/\/+$/, ''),
  };
}
